...
Gin uses the `validator` package internally, we can use the same for the **gRPC** gateway server. But the error responses are not so user friendly.

We can use the request struct generated by auto generated code use the builtin struct `errdetails.BadRequest_FieldViolation` from **genproto**.
